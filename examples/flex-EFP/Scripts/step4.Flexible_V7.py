...
def figure_atoms(atom,r,j):
    if "C" in atom and r < 0.8:  # LVS fix to accoount for LMOs in aromatic systems
        return j
    if "O" in atom and r < 0.8:
        ...
        return j
    elif "MG" in atom and r < 1.15:
        return j
    elif "FE" in atom and r < 1.15:
        return j
    else:
        return "Nolone"
...
def rotate_lmo_tensor(neflmo,nefpol,rot,xyz1,xyz2,points,ref_lmo,ref_pol):
    for pt in points:
        ...
        #if neflmo[pt][0] == 0.0 and neflmo[pt][1] == 0.0 and neflmo[pt][2] == 0.0:
        #    print('ERROR-ERROR')
        neflmo[pt][3] += 1.0
...
def wrt_ele(find_file,parm_file,xyz,nefdip,nefqua,nefoct,no_pol,no_disp,no_xr,fragname,orig_frag):
    string = ''

    for i in range(find_file.index("COORDINATES (BOHR)")+1):
        if 'EFP DATA' in parm_file[i]:
            string += 'PREPARED BY step4.Flexible_V7.py FROM '+ orig_frag + '\n'
            continue
        elif '$' in parm_file[i]:
            string += ' $'+fragname+'\n'
        else:
...
def rotate_parameter(reflines,coord_xyz2,no_pol,no_disp,no_xr,fragname, orig_frag):
...
#    ofile      = open(outputfile,'w')
    ...
    new_parm = ''
    new_parm += wrt_ele(find_file,parm_file,xyz,nefdip,nefqua,nefoct,no_pol,no_disp,no_xr,fragname,orig_frag)
...
#    parser.add_argument('-v', '--version', action='version', version='Flexible EFP ' + __version__)
    ...
    orig_frag = args.conf_a
    frag_parm = rotate_parameter(reflines1,np.array(coord_xyz2),args.no_pol,args.no_disp,args.no_xr,frag, orig_frag)
